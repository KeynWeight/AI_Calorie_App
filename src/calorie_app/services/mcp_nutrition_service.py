...
class IngredientMatchingAgent:
    ...
    def __init__(
        ...
        self.last_reset_time = asyncio.get_event_loop().time() if hasattr(asyncio, 'get_event_loop') else 0
        self.calls_per_minute_limit = SystemLimits.RATE_LIMIT_PER_MINUTE  # Conservative limit
    ...
    def _analyze_ingredient_name(self, ingredient_name: str) -> str:
        ...
        try:
            ...
            cooking_methods = [
                ...
                'pan-fried', 'deep-fried', 'broiled', 'braised', 'stewed'
            ]
            
            descriptors = [
                'fresh', 'frozen', 'canned', 'dried', 'organic', 'free-range', 'grass-fed',
                'raw', 'cooked', 'chopped', 'diced', 'sliced', 'whole', 'ground', 'crushed',
                'extra', 'virgin', 'unsalted', 'salted', 'low-fat', 'fat-free', 'lean'
            ]
...
"""
    ...
    async def find_batch_matches(self, batch_prompt: str) -> Optional[str]:
        ...
        try:
            logger.info(f"[LLM] Using simple LLM for batch ingredient matching (API calls used: {self.api_calls_made})")
...
Return valid JSON only - no explanations."""
    ...
    async def _run_simple_llm_async(self, prompt: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"[COMPLETE] Simple LLM completed. Total API calls this minute: {self.api_calls_made}")
            return result
    ...
    async def _run_batch_agent_async(self, batch_prompt: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"[COMPLETE] Batch agent completed. Total API calls this minute: {self.api_calls_made}")
            return result
...
""")
            ...
            batch_agent = create_react_agent(self.llm, self.agent_executor.tools, batch_prompt_template)
    ...
    async def find_best_match(self, ingredient_name: str) -> Optional[Dict[str, Any]]:
        ...
        """
        # Check cache first
        if self.enable_caching and self.successful_mappings and ingredient_name in self.successful_mappings:
        ...
        # Rate limiting check
        if not self._check_rate_limit():
            logger.warning(f"⚠️  Rate limit exceeded, skipping agent for: {ingredient_name}")
            return None
        
        try:
            logger.info(f"[AGENT] Using agent to find match for: {ingredient_name} (API calls used: {self.api_calls_made})")
            ...
            if result and "output" in result:
                ...
                if match:
                    ...
                    logger.info(f"[SUCCESS] Agent found match: {match.get('description', 'Unknown')} (ID: {match.get('fdcId')})")
                    return match
    ...
    async def _run_agent_async(self, ingredient_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                logger.info(f"[COMPLETE] Agent completed. Total API calls this minute: {self.api_calls_made}")
                return result
    ...
    def _parse_agent_result(self, agent_output: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            match = re.search(r'BEST_MATCH:\s*([^|]+)\|([^|]+)\|([^|\n]+)', agent_output)
            ...
            if id_match:
                return {
                    ...
                    "description": desc_match.group(1) if desc_match else "Agent selected",
                    "confidence": "medium",
                    "agent_reasoning": agent_output
                }
...
class MCPNutritionService:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to MCP services. Available tools: {len(self.tools)}")
            
            # Log available tools for debugging
            tool_names = []
            for tool in self.tools:
                tool_names.append(tool.name)
                logger.info(f"Available MCP tool: {tool.name} - {tool.description[:100]}...")
            ...
            if self.llm and LANGCHAIN_AVAILABLE:
                try:
                    self.agent = IngredientMatchingAgent(
                        ...
                        max_execution_time=self.agent_config.get("max_execution_time", 30.0),
                        enable_caching=self.agent_config.get("enable_caching", True)
                    )
                    # Update rate limit
                    self.agent.calls_per_minute_limit = self.agent_config.get("calls_per_minute_limit", 20)
                    logger.info(f"Ingredient matching agent initialized (max_iterations: {self.agent_config.get('max_iterations', 3)}, rate_limit: {self.agent_config.get('calls_per_minute_limit', 20)}/min)")
                except Exception as e:
    ...
    async def search_food(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        """
            ...
            if isinstance(result, str):
                try:
                    ...
                    if isinstance(parsed_result, dict) and "foods" in parsed_result:
                        ...
                        logger.warning(f"Unexpected parsed result format: {type(parsed_result)}")
                        return []
    ...
    async def search_and_cache_nutrition_data(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(f"[CACHE] Pre-caching nutrition data for {len(search_results)} '{query}' results")
                        ...
                        try:
                            ...
                            if food_details:
                                ...
                                if dynamic_nutrition:
                                    ...
                                    cached_count += 1
                                    logger.debug(f"[CACHE] Cached nutrition data for ID:{fdc_id}")
                        except Exception as e:
                            logger.warning(f"[CACHE] Failed to cache nutrition data for ID:{fdc_id}: {e}")
                    else:
                        logger.debug(f"[CACHE] Already cached: ID:{fdc_id}")
            
            logger.info(f"[CACHE] Successfully cached nutrition data for {cached_count}/{len(search_results)} results")
    ...
    async def get_food_details(self, food_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                logger.info(f"[RESOURCE] Attempting to fetch food details for ID: {food_id}")
                results = await self.mcp_client.get_resources("food-data-central", uris=[resource_uri])
                ...
                if results and len(results) > 0:
                    ...
                    import json
                    try:
                        data = json.loads(blob.text)  # Use .text instead of .as_string()
                        ...
                        try:
                            if hasattr(blob, 'as_string'):
                                ...
                                logger.debug(f"Successfully parsed food details for {food_id} (fallback)")
                                return data
                            elif hasattr(blob, 'contents') and blob.contents:
                                data = json.loads(blob.contents[0].text)
                                logger.debug(f"Successfully parsed food details for {food_id} (contents)")
                                return data
                            else:
                                logger.error(f"Failed to parse food details JSON for {food_id}: {parse_error}")
                                return None
                        except Exception as fallback_error:
                            logger.error(f"All parsing attempts failed for {food_id}: {fallback_error}")
                            return None
                else:
                    logger.warning(f"No results from food details resource for {food_id}")
                    return None
                    
            except Exception as resource_error:
                import traceback
                logger.warning(f"MCP resource approach failed for {food_id}: {str(resource_error)}")
    ...
    async def _get_food_details_direct_api(self, food_id: str) -> Optional[Dict[str, Any]]:
        """
                ...
                try:
                    with urllib.request.urlopen(full_url) as response:
                        if response.status == 200:
                            ...
                            logger.info(f"[DIRECT API] Successfully retrieved food details for {food_id}")
                            return data
                        else:
                            logger.warning(f"[DIRECT API] Failed with status {response.status} for food ID {food_id}")
                            return None
                except Exception as e:
    ...
    async def search_and_get_ingredient(
        ...
        try:
            ...
            logger.error(f"USDA ingredient search failed for {ingredient_name}: {str(e)}")
            return None
    ...
    async def get_complete_nutrition_data(
        ...
        try:
            ...
            logger.error(f"Complete nutrition data retrieval failed for {ingredient_name}: {str(e)}")
            return None
    ...
    async def get_nutrition_data_by_id(
        ...
        """
        try:
            logger.info(f"Getting nutrition data by ID for: {ingredient_name} (ID: {usda_id})")
            ...
            logger.error(f"Nutrition data retrieval by ID failed for {ingredient_name} (ID: {usda_id}): {str(e)}")
            return None
    ...
    def get_cached_nutrition_data(
        ...
        try:
            ...
            if weight_grams == baseline_nutrition.weight_grams:
                ...
                logger.info(f"[CACHE HIT] Using cached data for ID:{fdc_id} (same weight: {weight_grams}g)")
                return scaled_nutrition
            
            # Different weight, need to scale
            scale_factor = weight_grams / baseline_nutrition.weight_grams
            logger.info(f"[CACHE HIT] Scaling cached data for ID:{fdc_id} ({baseline_nutrition.weight_grams}g -> {weight_grams}g)")
            ...
            logger.error(f"Error retrieving cached nutrition data for {fdc_id}: {str(e)}")
            return None
    ...
    @cached_agent_response()
    async def _find_best_ingredient_match(self, ingredient_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.use_agent and self.agent and self.agent.agent_executor:
                logger.info(f"Using intelligent agent for ingredient matching: {ingredient_name}")
                agent_result = await self.agent.find_best_match(ingredient_name)
                if agent_result:
                    return agent_result
                else:
                    logger.info(f"Agent failed, falling back to rule-based search for: {ingredient_name}")
            
            # Strategy 2: Fallback to rule-based approach
            return await self._rule_based_ingredient_search(ingredient_name)
            
        except Exception as e:
            logger.error(f"Error finding best ingredient match for {ingredient_name}: {str(e)}")
            return None
    
    @cached_agent_response()
    async def _rule_based_ingredient_search(self, ingredient_name: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            search_results = await self.search_food(ingredient_name, max_results=SystemLimits.MAX_SEARCH_RESULTS)
            ...
            if not search_results:
                ...
                if simplified_name != ingredient_name:
                    logger.info(f"Trying simplified search: '{simplified_name}' for '{ingredient_name}'")
                    search_results = await self.search_food(simplified_name, max_results=SystemLimits.MAX_SEARCH_RESULTS)
            ...
            if not search_results:
                ...
                if len(words) > 1:
                    # Try first word only
                    logger.info(f"Trying first word: '{words[0]}' for '{ingredient_name}'")
                    search_results = await self.search_food(words[0], max_results=SystemLimits.MAX_SEARCH_RESULTS)
                    
                    if not search_results and len(words) > 2:
                        # Try last word if still no results
                        logger.info(f"Trying last word: '{words[-1]}' for '{ingredient_name}'")
                        search_results = await self.search_food(words[-1], max_results=SystemLimits.MAX_SEARCH_RESULTS)
            ...
            logger.info(f"Rule-based match for '{ingredient_name}': {best_match.get('description', 'Unknown')} (ID: {best_match.get('fdcId')})")
    ...
    def _rank_search_results(self, query: str, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for result in results:
            ...
            if any(processed in description for processed in ['prepared', 'seasoned', 'flavored', 'canned with']):
                score -= 2
        ...
        for i, (score, result) in enumerate(scored_results[:3]):
            logger.debug(f"  {i+1}. {result.get('description', 'Unknown')} (Score: {score})")
    ...
    def get_agent_status(self) -> Dict[str, Any]:
        ...
        return {
            "agent_available": self.agent is not None and self.agent.agent_executor is not None,
