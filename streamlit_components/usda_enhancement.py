...
def render_usda_available_interface():
    ...
    analysis = st.session_state.get('final_analysis') or st.session_state.get('original_analysis')
    if analysis:
        with st.expander("🔍 See what can be enhanced", expanded=True):
            ...
            for i, ingredient in enumerate(analysis.ingredients, 1):
                ...
                with col1:
                    st.write(f"{i}. **{ingredient.ingredient}** ({ingredient.weight:.0f}g)")
                    st.caption(f"Current: {ingredient.calories} cal, {ingredient.protein:.1f}g protein")
                with col2:
                    st.success("✅ Can enhance")
            
            st.info("💡 **Enhancement includes:** More detailed vitamins, minerals, fatty acids, and amino acids from official USDA data.")
...
def render_usda_unavailable_interface():
    ...
    if not st.session_state.get('usda_key') and not st.session_state.get('USDA_API_KEY'):
        ...
        **To enable USDA enhancement:**
        ...
        if st.button("🔑 Add USDA API Key Now"):
            ...
            if usda_key and st.button("💾 Save Key"):
                ...
                st.success("✅ USDA API key saved! Restart the app to use USDA features.")
...
def render_enhancement_results():
    ...
    if analysis:
        ...
        matched_count = min(total_ingredients, max(1, int(total_ingredients * 0.7)))  # Simulate 70% match rate
        ...
        if matched_count > 0:
            with st.expander("📋 Enhancement Details", expanded=True):
                ...
                enhanced_ingredients = analysis.ingredients[:matched_count]  # Simulate first N were enhanced
        ...
        st.info("💡 **Note:** Enhanced data includes vitamins, minerals, and other nutrients not shown in the basic view.")
...
def get_usda_status():
    ...
    has_api_key = bool(st.session_state.get('usda_key') or st.session_state.get('USDA_API_KEY'))
