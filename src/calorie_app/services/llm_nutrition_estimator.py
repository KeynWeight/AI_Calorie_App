...
class LLMNutritionEstimator:
    ...
    def __init__(self, model: str = ModelDefaults.NUTRITION_ESTIMATION_MODEL, api_key: str = None, base_url: str = None):
...
Base your estimate on standard USDA nutrition data. Be as accurate as possible."""
            ...
            logger.info(f"[LLM NUTRITION REQUEST] Requested nutrition estimate for: {ingredient_name} ({weight}g)")
            ...
            logger.info(f"[LLM RAW RESPONSE] Response for {ingredient_name}: {response_preview}")
            
            # Try to log structured JSON response
            try:
                parsed_preview = json.loads(response_content)
                logger.info(f"[LLM STRUCTURED RESPONSE] Parsed nutrition data for {ingredient_name}:")
                logger.info(f"[LLM STRUCTURED RESPONSE]   Calories: {parsed_preview.get('calories', 'N/A')}")
                logger.info(f"[LLM STRUCTURED RESPONSE]   Protein: {parsed_preview.get('protein', 'N/A')}g")
            ...
            try:
                ...
                logger.debug(f"[LLM PARSED] {ingredient_name}: {nutrition_data.get('calories', 'N/A')} cal, {nutrition_data.get('protein', 'N/A')}g protein, {nutrition_data.get('carbohydrates', 'N/A')}g carbs, {nutrition_data.get('fat', 'N/A')}g fat")
                
                # Validate the response structure
                validation_result = NutritionValidator.validate_nutrition_response(nutrition_data)
                if not validation_result['is_valid']:
                    logger.warning(f"Invalid nutrition response format for {ingredient_name}: {validation_result}")
                    return self._create_fallback_ingredient(ingredient_name, weight)
                
                # Create Ingredient object
                ingredient = Ingredient(**nutrition_data)
                logger.info(f"[LLM SUCCESS] Successfully estimated nutrition for {ingredient_name}: {ingredient.calories} cal")
                return ingredient
    ...
    def _create_fallback_ingredient(self, ingredient_name: str, weight: float) -> Ingredient:
    ...
    def estimate_multiple_ingredients(
        self, 
        ingredients_with_weights: Dict[str, float]
...
{{
    ...
    "sodium": 100.0
            ...
            logger.debug(f"[LLM CONTEXT] Context-refined response for {ingredient_name} in {dish_context}: {response_preview}")
            ...
            if validation_result['is_valid']:
                ingredient = Ingredient(**nutrition_data)
                logger.debug(f"[LLM CONTEXT SUCCESS] {ingredient_name} with context '{dish_context}': {ingredient.calories} cal")
                return ingredient
            else:
                logger.warning(f"[LLM CONTEXT FALLBACK] Invalid context response for {ingredient_name}, falling back to regular estimation")
