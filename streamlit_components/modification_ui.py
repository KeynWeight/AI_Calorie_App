...
def render_modification_interface(analysis) -> Optional[UserModification]:
    ...
    st.write("Make changes to dish name, ingredients, or weights. Changes will be processed by our AI system.")
    ...
    with st.expander("🏷️ Edit Dish Name", expanded=True):
        ...
        new_name = st.text_input(
            "Dish Name",
            value=current_name,
            help="Enter a new name for this dish"
        )
    ...
    with st.expander("🥕 Modify Ingredients", expanded=True):
        ...
        for i, ingredient in enumerate(analysis.ingredients):
            ...
            with col1:
                ...
                st.write(f"Current: {ingredient.weight:.0f}g, {ingredient.calories} cal")
            ...
            with col2:
                ...
                if abs(new_weight - ingredient.weight) > 0.1:  # Changed significantly
                    ...
                    cal_ratio = new_weight / ingredient.weight if ingredient.weight > 0 else 0
    ...
    with st.expander("📏 Edit Portion Size"):
        st.info("💡 **Note:** This only changes the portion description text. To scale nutrition values proportionally, manually adjust individual ingredient weights above.")
...
def render_modification_summary(modifications: Dict, original_analysis):
    ...
    if 'dish_name' in modifications:
        changes_made.append(f"🏷️ **Dish name:** {original_analysis.dish_name} → {modifications['dish_name']}")
    ...
    if 'ingredient_weight_changes' in modifications:
        ...
        for ingredient, new_weight in modifications['ingredient_weight_changes'].items():
            ...
            original_weight = next(
                (ing.weight for ing in original_analysis.ingredients if ing.ingredient == ingredient),
                0
            )
            change = new_weight - original_weight
            changes_made.append(f"   • {ingredient}: {original_weight:.0f}g → {new_weight:.0f}g ({change:+.0f}g)")
    ...
    if 'portion_size' in modifications:
        changes_made.append(f"📏 **Portion size:** {original_analysis.portion_size} → {modifications['portion_size']}")
...
def estimate_modification_impact(modifications: Dict, original_analysis):
    ...
    if 'ingredient_weight_changes' in modifications:
        for ingredient, new_weight in modifications['ingredient_weight_changes'].items():
            ...
            original_ing = next(
                (ing for ing in original_analysis.ingredients if ing.ingredient == ingredient),
                None
            )
            if original_ing:
                if original_ing.weight > 0:
                    ...
                    if abs(calorie_change) > 10:  # Only show significant changes
                        impact_notes.append(f"• {ingredient}: {calorie_change:+.0f} calories")
    
    # Estimate from removed ingredients
    if 'ingredients_to_remove' in modifications:
        for ingredient_name in modifications['ingredients_to_remove']:
            removed_ing = next(
                (ing for ing in original_analysis.ingredients if ing.ingredient == ingredient_name),
                None
            )
            if removed_ing:
                estimated_calorie_change -= removed_ing.calories
                impact_notes.append(f"• Remove {ingredient_name}: -{removed_ing.calories} calories")
    
    # Note about added ingredients
    if 'ingredients_to_add' in modifications:
        total_added_weight = sum(modifications['ingredients_to_add'].values())
        impact_notes.append(f"• Added {len(modifications['ingredients_to_add'])} ingredients ({total_added_weight:.0f}g total) - calories will be calculated by AI")
    ...
    st.info("🤖 **Note:** Final nutrition values will be recalculated by our AI system based on your modifications.")

def clear_modification_state():
    """Clear modification-related session state."""
    keys_to_clear = [key for key in st.session_state.keys() if key.startswith(('new_ingredients', 'new_ingredient_weights', 'weight_', 'remove_', 'new_weight_'))]
    ...
    with col1:
        if st.button("🥑 Add Avocado"):
            ...
            if "Avocado" not in st.session_state.new_ingredients:
                ...
                st.session_state.new_ingredient_weights["Avocado"] = 50.0  # Default avocado weight
                st.rerun()
    ...
    with col2:
        if st.button("🧀 Add Cheese"):
            ...
            if "Cheese" not in st.session_state.new_ingredients:
                ...
                st.session_state.new_ingredient_weights["Cheese"] = 30.0  # Default cheese weight
                st.rerun()
    ...
    with col3:
        if st.button("🌶️ Add Hot Sauce"):
            ...
            if "Hot Sauce" not in st.session_state.new_ingredients:
                ...
                st.session_state.new_ingredient_weights["Hot Sauce"] = 10.0  # Default hot sauce weight
                st.rerun()
    
    with col4:
        if st.button("🥗 Make it Salad"):
            # Suggest reducing portions and adding vegetables
            st.info("💡 Consider reducing portions and adding lettuce, tomatoes, and cucumber!")
...
def validate_modifications(modifications: Dict) -> tuple[bool, List[str]]:
    ...
    if 'ingredients_to_add' in modifications:
        ...
        for ingredient, weight in ingredients.items():
            if weight <= 0:
                errors.append(f"Weight for new ingredient '{ingredient}' must be positive")
            elif weight > 1000:
                errors.append(f"Weight for new ingredient '{ingredient}' seems too high (>1000g)")
