...
class NutritionWorkflow:
    ...
    def __init__(
        ...
        self.vision_service = VisionService(
            model_name=vision_model, api_key=vision_api_key, base_url=vision_base_url
        )
        ...
        self.nutrition_service = NutritionService(
            ...
            or llm_model,  # Default to main LLM if not specified
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"await_human": "await_human_validation", "error": "handle_error"},
        )
        ...
        workflow.add_conditional_edges(
            ...
            {"handle_usda": "handle_usda_query", "finalize": "finalize_response"},
        )
            ...
            if not self._mcp_initialized:
                ...
                try:
                    success = loop.run_until_complete(
                        self.nutrition_service.initialize_mcp()
                    )
                    self._mcp_initialized = success
                    logger.info(
                        f"MCP initialization: {'successful' if success else 'failed'}"
                    )
                finally:
                    loop.close()
    ...
    def _analyze_image_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            log_workflow_step(
                "Image Analysis", "Starting", f"image: {state.image_path}", logger
            )
            ...
            if not analysis_result:
                ...
                if state.retry_count >= 2:
                    ...
                    logger.warning(
                        f"Retrying image analysis (attempt {state.retry_count})"
                    )
            ...
            log_workflow_step(
                "Image Analysis",
                "Complete",
                f"identified: {analysis_result.dish_name}",
                logger,
            )
            return {
    ...
    def _apply_modifications_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            ...
            modified_analysis = self.nutrition_service.apply_user_modifications(
                state.original_analysis, user_mods
            )
    ...
    def _generate_response_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        try:
            ...
            natural_response = self.formatting_service.generate_natural_response(
                analysis_to_use, state.validation_result
            )
    ...
    def _await_human_validation_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        interrupt_payload = {
            ...
            "dish_name": state.original_analysis.dish_name
            if state.original_analysis
            else "Unknown",
            "total_calories": state.original_analysis.total_calories
            if state.original_analysis
            else 0,
            "ingredients_count": len(state.original_analysis.ingredients)
            if state.original_analysis
            else 0,
            "ingredients": [
                {"name": ing.ingredient, "weight": ing.weight, "calories": ing.calories}
                for ing in state.original_analysis.ingredients
            ]
            if state.original_analysis
            else [],
            "prompt": (
    ...
    def _finalize_response_node(self, state: WorkflowState) -> Dict[str, Any]:
        ...
        final_analysis = (
            state.usda_enhanced_analysis
            or state.final_analysis
            or state.original_analysis
        )
        ...
        if (
            hasattr(state, "usda_matches_count")
            and state.usda_matches_count is not None
        ):
            ...
            logger.info(
                f"Workflow finalized with USDA enhancement: {state.usda_matches_count}/{state.total_ingredients} ingredients enhanced"
            )
        else:
...
"""
    ...
    def _route_human_decision(self, state: WorkflowState) -> str:
        ...
        if state.user_modifications:
            ...
            if isinstance(state.user_modifications, dict):
                ...
                has_modifications = (
                    mods.get("dish_name")
                    or mods.get("ingredients_to_add")
                    or mods.get("ingredients_to_remove")
                    or mods.get("ingredient_weight_changes")
                    or mods.get("portion_size")
                )
                ...
                logger.debug(
                    f"[WORKFLOW] User modifications (object): {type(state.user_modifications)}"
                )

        if has_modifications:
            logger.info(
                "[WORKFLOW] -> User has modifications, routing to apply_modifications"
            )
    ...
    def _should_handle_usda_query(self, state: WorkflowState) -> str:
        ...
        logger.debug(
            f"[WORKFLOW] USDA query routing: wants_usda={wants_usda}, mcp_initialized={self._mcp_initialized}"
        )
    ...
    def analyze_dish(
        ...
        try:
            log_workflow_step(
                "Workflow", "Starting", f"image: {Path(image_path).name}", logger
            )
            ...
            results = {
                ...
                or result_state.original_analysis,
            ...
            if results["success"] and results["dish_analysis"]:
                results["health_insights"] = (
                    self.formatting_service.generate_health_insights(
                        results["dish_analysis"]
                    )
                )

            log_workflow_step(
                "Workflow", "Completed", f"status: {result_state.status}", logger
            )
            return results
    ...
    def resume_workflow_with_modifications(
        self, modifications: UserModification, thread_id: str = "default"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if (
                ...
                and result_state.original_analysis
                and result_state.user_modifications
            ):
                modification_summary = (
                    self.formatting_service.generate_modification_summary(
                        result_state.original_analysis, result_state.final_analysis
                    )
                )
            else:
            ...
            return {
                ...
                if result_state.final_analysis
                else [],
            }
    ...
    def start_analysis(
        self, image_path: str, thread_id: str = "default"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            log_workflow_step(
                "Analysis", "Starting", f"image: {Path(image_path).name}", logger
            )

            # Initial state
            initial_state = WorkflowState(
                image_path=image_path, status=AnalysisState.PENDING
            )
                ...
                if "__interrupt__" in step:
                    ...
                    if current_state and current_state.values:
                        return {
                            ...
                            "validation_prompt": current_state.values.get(
                                "human_validation_prompt", "Please review the analysis"
                            ),
                            "original_analysis": current_state.values.get(
                                "original_analysis"
                            ),
                            "validation_result": current_state.values.get(
                                "validation_result"
                            ),
                            "thread_id": thread_id,
                        }
    ...
    def submit_human_validation(
        ...
        try:
            log_workflow_step(
                "Human Validation", "Submitting", f"approved: {approved}", logger
            )
            ...
            self.app.update_state(
                config, resume_input, as_node="await_human_validation"
            )
            ...
            return {
                ...
                "status": result_state.status,
                "dish_analysis": (
                    getattr(result_state, "usda_enhanced_analysis", None)
                    or result_state.final_analysis
                    or result_state.original_analysis
                ),
                "natural_response": result_state.natural_response,
                "formatted_tone_response": getattr(
                    result_state, "formatted_tone_response", ""
                ),
                "usda_query_available": getattr(
                    result_state, "usda_query_available", False
                ),
                ...
                "usda_enhancement_summary": getattr(
                    result_state, "usda_enhancement_summary", ""
                ),
