...
def render_model_config():
    ...
    with st.expander("🤖 Model Configuration", expanded=False):
        ...
        vision_model = st.selectbox(
            "Vision Model",
            vision_models,
            index=vision_models.index(current_vision)
...
def render_system_status():
    ...
    with st.expander("📊 System Status", expanded=True):
        # API Status
        openrouter_key = st.session_state.get("openrouter_key", "") or os.getenv(
            "OPENROUTER_API_KEY", ""
        )
        ...
        if usda_key:
            ...
            mcp_server_exists = os.path.exists(
                "food-data-central-mcp-server/dist/index.js"
            )
