    ...
    for key in analysis_keys[2:]:  # Skip stage and thread_id
        if key in st.session_state:
            st.session_state[key] = (
                None
                if "usda_matches" not in key and "wants_usda" not in key
                else (0 if "matches" in key else False)
            )
...
def get_current_analysis():
    ...
    return (
        st.session_state.get("enhanced_analysis")
        or st.session_state.get("final_analysis")
        or st.session_state.get("original_analysis")
    )
...
def get_modification(ingredient_name: str, field: str, default=None):
    ...
    return (
        st.session_state.get("modifications", {})
        .get(ingredient_name, {})
        .get(field, default)
    )
