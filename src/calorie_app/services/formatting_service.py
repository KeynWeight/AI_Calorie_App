...
Make it sound natural and encouraging while including ALL nutritional information. Add a brief positive comment about the dish if appropriate."""
    ...
    def generate_natural_response(
        self, nutrition_data: DishNutrition, validation_result=None
    ) -> str:
        """
        ...
        try:
            ...
            confidence_text = self._format_confidence_info(
                nutrition_data, validation_result
            )
            ...
            messages = [
                ...
                HumanMessage(
                    content=f"""Format this nutrition data into natural language:
...
Make it conversational and informative. Include the confidence information naturally in the response."""
            ...
            logger.debug(
                f"[LLM] System message length: {len(messages[0].content) if messages else 0}"
            )
            logger.debug(
                f"[LLM] User message length: {len(messages[1].content) if len(messages) > 1 else 0}"
            )
            ...
            logger.info(
                f"[FINAL LLM] Generated final response for '{nutrition_data.dish_name}' ({len(formatted_response)} characters)"
            )
    ...
    def _format_confidence_info(
        ...
        if final_confidence >= NutritionThresholds.HIGH_CONFIDENCE:
            ...
            confidence_note = (
                "This analysis has some uncertainty - please review carefully."
            )
        else:
            confidence_description = "low confidence"
            confidence_note = (
                "This analysis is uncertain - please verify the information."
            )

        confidence_parts.append(
            f"Analysis Confidence Score: {final_confidence:.0%} ({confidence_description})"
        )
        ...
        if (
            validation_result
            and hasattr(validation_result, "warnings")
            and validation_result.warnings
        ):
            if len(validation_result.warnings) <= 2:
                confidence_parts.append(
                    f"Note: {'; '.join(validation_result.warnings)}"
                )
            else:
                confidence_parts.append(
                    f"Note: {len(validation_result.warnings)} quality concerns detected."
                )
    ...
    def generate_modification_summary(
        self, original: DishNutrition, modified: DishNutrition
    ) -> str:
        """
        ...
        try:
            ...
            messages = [
                SystemMessage(
                    content="""You are explaining changes made to a nutrition analysis.
...
3. Whether changes are positive or negative for health"""
                ...
                HumanMessage(
                    content=f"""Summarize the changes made to this dish analysis:
    ...
    def generate_health_insights(self, nutrition_data: DishNutrition) -> list:
        ...
        if total_calories > 0:
            ...
            if protein_percent > NutritionThresholds.HIGH_PROTEIN_PERCENT:
                insights.append(
                    "✓ Excellent protein content for muscle maintenance and satiety"
                )
            ...
            if fat_percent > NutritionThresholds.HIGH_FAT_PERCENT:
                insights.append(
                    "⚠ High fat content - consider reducing oils or fatty ingredients"
                )

        # Sodium check
        if (
            nutrition_data.total_sodium
            and nutrition_data.total_sodium > NutritionThresholds.HIGH_SODIUM_MG
        ):
            insights.append(
                "⚠ High sodium content - consider reducing salt or processed ingredients"
            )

        # Fiber check
        if (
            nutrition_data.total_fiber
            and nutrition_data.total_fiber > NutritionThresholds.HIGH_FIBER_THRESHOLD
        ):
            insights.append("✓ Good fiber content supports digestive health")
        elif (
            nutrition_data.total_fiber
            and nutrition_data.total_fiber < NutritionThresholds.LOW_FIBER_THRESHOLD
        ):
            insights.append(
                "⚠ Low fiber - adding vegetables or whole grains would be beneficial"
            )

        # Calorie assessment
        if nutrition_data.total_calories > NutritionThresholds.HIGH_CALORIE_THRESHOLD:
            insights.append(
                "⚠ High-calorie dish - consider smaller portions or lighter sides"
            )
        elif nutrition_data.total_calories < NutritionThresholds.LOW_CALORIE_THRESHOLD:
            insights.append(
                "ℹ Light dish - you might want to pair with other foods for a complete meal"
            )
