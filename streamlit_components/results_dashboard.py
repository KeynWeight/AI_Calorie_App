...
def render_hero_section(analysis):
    ...
    if st.session_state.get('usda_matches', 0) > 0:
        ...
        st.success(f"✨ Enhanced with USDA data: {matches}/{total_ingredients} ingredients")
...
def render_main_dashboard(analysis):
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Overview", "🥕 Ingredients", "🔬 Micronutrients", "📈 Charts", "🔍 Analysis"])
...
def render_micronutrients_section(analysis):
    ...
    enhanced_ingredients = [ing for ing in analysis.ingredients if ing.complete_nutrition]
    ...
    for ingredient in enhanced_ingredients:
        if ingredient.complete_nutrition:
            ...
            for nutrient_id, nutrient in nutrients.items():
                ...
                if any(vitamin in name for vitamin in ['vitamin', 'folate', 'niacin', 'riboflavin', 'thiamin', 'choline', 'betaine']):
                    vitamins[nutrient.name] = nutrient
                elif any(mineral in name for mineral in ['calcium', 'iron', 'magnesium', 'phosphorus', 'potassium', 'sodium', 'zinc', 'copper', 'manganese', 'selenium']):
                    minerals[nutrient.name] = nutrient
                elif nutrient.name.lower() not in ['energy', 'protein', 'total lipid (fat)', 'carbohydrate, by difference']:
...
def render_overview_tab(analysis):
    ...
    with col1:
        ...
        info_data = {
            ...
            "Analysis Confidence": f"{analysis.confidence_score:.1%}" if analysis.confidence_score else "N/A"
        }
...
def render_ingredients_tab(analysis):
    ...
    for i, ing in enumerate(analysis.ingredients, 1):
        ...
        cal_percent = (ing.calories / analysis.total_calories * 100) if analysis.total_calories > 0 else 0
        ...
        data.append({
            ...
            'Fat (g)': f"{ing.fat:.1f}",
            'Cal/100g': f"{(ing.calories / ing.weight * 100):.0f}" if ing.weight > 0 else "0"
        })
    ...
    st.dataframe(
        ...
        column_config={
            ...
            'Cal/100g': st.column_config.TextColumn('Cal/100g', width="small", help="Calorie density")
        }
    )
    ...
    sorted_ingredients = sorted(analysis.ingredients, key=lambda x: x.calories, reverse=True)
...
def render_charts_tab(analysis):
    ...
    chart_type = st.selectbox(
        "Select chart type:",
        ["Macronutrient Pie Chart", "Calorie Distribution", "Ingredient Comparison", "Nutrient Profile"],
        index=0
    )
...
def render_calorie_distribution_chart(analysis):
    ...
    try:
        ...
        for ing in sorted(analysis.ingredients, key=lambda x: x.calories, reverse=True):
            percent = (ing.calories / analysis.total_calories * 100) if analysis.total_calories > 0 else 0
            st.progress(percent / 100, text=f"{ing.ingredient}: {ing.calories} cal ({percent:.1f}%)")
...
def render_ingredient_comparison_chart(analysis):
    ...
    metric = st.selectbox("Compare ingredients by:", ["Calories", "Protein", "Carbs", "Fat", "Weight"])
...
def render_nutrient_profile_chart(analysis):
    ...
    nutrients = {
        ...
        'Sodium (mg)': (analysis.total_sodium or 0) / 10  # Scale down for visualization
    }
...
def render_analysis_tab(analysis):
    ...
    with col1:
        ...
        st.write(f"**Analysis Method:** {'USDA Enhanced' if st.session_state.get('usda_matches', 0) > 0 else 'AI Vision Analysis'}")
        st.write(f"**Confidence Score:** {analysis.confidence_score:.1%}" if analysis.confidence_score else "N/A")
        st.write(f"**Number of Ingredients:** {len(analysis.ingredients)}")
        
        if st.session_state.get('usda_matches', 0) > 0:
            matches = st.session_state.get('usda_matches', 0)
            st.write(f"**USDA Matches:** {matches}/{len(analysis.ingredients)} ingredients")
...
def render_simple_macro_breakdown(analysis):
    ...
    if total_macro_cal > 0:
        st.write(f"🥩 **Protein:** {protein_cal:.0f} cal ({protein_cal/total_macro_cal*100:.0f}%)")
        st.progress(protein_cal/total_macro_cal)
        
        st.write(f"🌾 **Carbohydrates:** {carb_cal:.0f} cal ({carb_cal/total_macro_cal*100:.0f}%)")
        st.progress(carb_cal/total_macro_cal)
        
        st.write(f"🧈 **Fat:** {fat_cal:.0f} cal ({fat_cal/total_macro_cal*100:.0f}%)")
