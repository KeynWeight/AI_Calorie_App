    ...
    def analyze_dish_image(
        ...
        try:
            ...
            standardized_logger.log_structured_data(
                response.content, "VLM", f"VisionService-{self.model_name}"
            )
            ...
            # Validate confidence
            if (
                hasattr(nutrition_data, "confidence_score")
                and nutrition_data.confidence_score
                and nutrition_data.confidence_score < confidence_threshold
            ):
            ...
            log_vision_result(
                ...
                len(nutrition_data.ingredients),
                nutrition_data.confidence_score,
                logger,
            )
            ...
            for i, ingredient in enumerate(nutrition_data.ingredients, 1):
                ...
                logger.debug(
                    f"     Calories: {ingredient.calories}, Protein: {ingredient.protein}g, Carbs: {ingredient.carbohydrates}g, Fat: {ingredient.fat}g"
                )
                if ingredient.sodium:
                    logger.debug(
                        f"     Sodium: {ingredient.sodium}mg, Fiber: {ingredient.fiber}g, Sugar: {ingredient.sugar}g"
                    )

            # Log VLM totals before local calculation
            logger.debug(
                f"[VLM TOTALS] VLM provided: {nutrition_data.total_calories} cal, {nutrition_data.total_protein}g protein, {nutrition_data.total_carbohydrates}g carbs, {nutrition_data.total_fat}g fat"
            )

            # Calculate totals locally using NutritionCalculator (not trusting VLM)
            calculated_totals = self.calculator.calculate_dish_totals(
            ...
            logger.info(
                f"  Calories: {nutrition_data.total_calories} -> {calculated_totals['total_calories']}"
            )
            logger.info(
                f"  Protein: {nutrition_data.total_protein}g -> {calculated_totals['total_protein']}g"
            )
            logger.info(
                f"  Carbs: {nutrition_data.total_carbohydrates}g -> {calculated_totals['total_carbohydrates']}g"
            )
            logger.info(
                f"  Fat: {nutrition_data.total_fat}g -> {calculated_totals['total_fat']}g"
            )
            logger.info(
                f"  Fiber: {nutrition_data.total_fiber}g -> {calculated_totals['total_fiber']}g"
            )
            logger.info(
                f"  Sugar: {nutrition_data.total_sugar}g -> {calculated_totals['total_sugar']}g"
            )
            logger.info(
                f"  Sodium: {nutrition_data.total_sodium}mg -> {calculated_totals['total_sodium']}mg"
            )

            # Update the nutrition data with locally calculated totals
            nutrition_data.total_calories = calculated_totals["total_calories"]
            nutrition_data.total_carbohydrates = calculated_totals[
                "total_carbohydrates"
            ]
            ...
            # Log final result
            logger.info(
                f"[FINAL] Analysis complete: {nutrition_data.dish_name} - {nutrition_data.total_calories} calories"
            )

            logger.info(
                f"[CALC] Local calculation: {calculated_totals['total_calories']} calories (VLM provided estimates, using our calculation)"
            )
...
- sodium: milligrams"""
    ...
    def _clean_json_response(self, response_content: str) -> Optional[dict]:
        ...
        try:
            ...
            json_text = re.sub(
                r"([{,]\s*)([a-zA-Z_][a-zA-Z0-9_]*)\s*:", r'\1"\2":', json_text
            )  # Quote keys
