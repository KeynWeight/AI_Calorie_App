...
def setup_workflow():
    ...
    try:
        ...
        config = {
            ...
            "vision_api_key": st.session_state.get("openrouter_key")
            or os.getenv("OPENROUTER_API_KEY"),
            "vision_base_url": os.getenv("OPENROUTER_API_URL"),
            "llm_model": st.session_state.get(
                "llm_model", "meta-llama/llama-3.2-3b-instruct:free"
            ),
            "llm_api_key": st.session_state.get("openrouter_key")
            or os.getenv("OPENROUTER_API_KEY"),
...
def render_enhancing_stage():
    ...
    try:
        ...
        result = workflow.submit_human_validation(
            thread_id=thread_id, approved=True, modifications=None, wants_usda_info=True
        )
        ...
        if result.get("success"):
            ...
            if enhanced_analysis:
                print(
                    f"Enhanced ingredients count: {len(enhanced_analysis.ingredients)}"
                )
                for i, ing in enumerate(enhanced_analysis.ingredients):
                    print(
                        f"  {i + 1}. {ing.ingredient}: {ing.calories}cal, {ing.protein}g protein, {ing.carbohydrates}g carbs, {ing.fat}g fat"
                    )
...
def render_debug_info():
    ...
    with st.expander("🐛 Debug Information", expanded=False):
        ...
        debug_state = {
            ...
            if not k.startswith("workflow_obj")
        }  # Don't show workflow object
        st.json(debug_state)
        ...
        st.write(
            f"- Analysis Stage: {st.session_state.get('analysis_stage', 'upload')}"
        )
