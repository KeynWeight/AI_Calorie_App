...
class NutrientInfo(BaseModel):
    ...
    amount_per_100g: Optional[float] = Field(
        None, description="Original amount per 100g"
    )
...
class DynamicNutritionData(BaseModel):
    ...
    nutrients: Dict[str, NutrientInfo] = Field(
        default_factory=dict, description="All nutrients by ID"
    )
    raw_usda_data: Optional[Dict[str, Any]] = Field(
        None, description="Complete raw USDA response"
    )
...
class Ingredient(BaseModel):
    ...
    complete_nutrition: Optional[DynamicNutritionData] = Field(
        None, description="Complete USDA nutrition data"
    )
    ...
    @classmethod
    def from_dynamic_nutrition(
        cls, nutrition_data: DynamicNutritionData
    ) -> "Ingredient":
...
class DishNutrition(BaseModel):
    ...
    portion_size: str = Field(
        default="1 serving", description="Portion size description"
    )
    confidence_score: Optional[float] = Field(
        0.8, ge=0, le=1, description="Analysis confidence"
    )
...
class UserModification(BaseModel):
    ...
    ingredients_to_remove: Optional[List[str]] = Field(
        None, description="Ingredient names to remove"
    )
    ingredient_weight_changes: Optional[Dict[str, float]] = Field(
        None, description="Weight changes for existing ingredients (numeric grams)"
    )
    ingredients_to_add: Optional[Dict[str, float]] = Field(
        None,
        description="New ingredients to add with their weights (ingredient_name -> weight_in_grams)",
    )
...
class ValidationResult(BaseModel):
    ...
    suggestions: List[str] = Field(
        default_factory=list, description="Health suggestions"
    )
