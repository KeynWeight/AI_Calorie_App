...
def render_hero_metrics(analysis):
    ...
    with col4:
        st.metric(
            label="🧈 Fat", value=f"{analysis.total_fat:.1f}g", help="Total fat content"
        )
    ...
    for i, ing in enumerate(ingredients, 1):
        with st.expander(f"{i}. {ing.ingredient} ({ing.weight:.0f}g)"):
            ...
            with col2:
                ...
                st.write(
                    f"• Sodium: {ing.sodium:.0f}mg" if ing.sodium else "• Sodium: 0mg"
                )
...
def render_macro_chart(analysis):
    ...
    try:
        ...
        if total_cal > 0:
            ...
            st.write(
                f"• Protein: {protein_cal:.0f} cal ({protein_cal / total_cal * 100:.0f}%)"
            )
...
def render_health_insights(analysis):
    ...
    total_weight = (
        sum(ing.weight for ing in analysis.ingredients) if analysis.ingredients else 0
    )
    if total_weight > 0:
...
def render_comparison_view(original_analysis, modified_analysis):
    ...
    with col2:
        ...
        protein_delta = (
            modified_analysis.total_protein - original_analysis.total_protein
        )
        carb_delta = (
            modified_analysis.total_carbohydrates
            - original_analysis.total_carbohydrates
        )
        fat_delta = modified_analysis.total_fat - original_analysis.total_fat

        st.metric(
            "Calories", f"{modified_analysis.total_calories:,}", delta=f"{cal_delta:+,}"
        )
        st.metric(
            "Protein",
            f"{modified_analysis.total_protein:.1f}g",
            delta=f"{protein_delta:+.1f}g",
        )
        st.metric(
            "Carbs",
            f"{modified_analysis.total_carbohydrates:.1f}g",
            delta=f"{carb_delta:+.1f}g",
        )
        st.metric(
            "Fat", f"{modified_analysis.total_fat:.1f}g", delta=f"{fat_delta:+.1f}g"
        )
