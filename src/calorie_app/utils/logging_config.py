...
class StandardizedLogger:
    ...
    def setup_logging(self) -> None:
        ...
        logger.info(f"[SYS] Logging configured - Console: {self.console_level}, File: {self.file_level}")
    ...
    def log_structured_data(
        ...
        logger.info(f"[{prefix}] {operation}: {data[:100]}{'...' if len(data) > 100 else ''}")
    ...
    def end_performance_timer(self, operation: str, logger: Optional[logging.Logger] = None) -> float:
    ...
    def _log_to_file(self, content: str, filename: str, source: str) -> None:
        ...
        try:
            ...
            logging.getLogger(self.app_package).warning(f"[ERR] Failed to log to {filename}: {e}")
...
def log_vision_result(dish_name: str, ingredients_count: int, confidence: float, logger: logging.Logger) -> None:
    """Log vision analysis results with standardized format."""
    logger.info(f"[VIS] Analyzed '{dish_name}': {ingredients_count} ingredients (confidence: {confidence:.1f})")


def log_api_call(service: str, operation: str, duration: float, success: bool, logger: logging.Logger) -> None:
...
def log_workflow_step(step: str, status: str, details: str = "", logger: logging.Logger = None) -> None:
