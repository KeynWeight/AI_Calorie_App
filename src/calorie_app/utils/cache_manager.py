...
class CacheManager:
    ...
    def get_all_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "vlm_cache": {
                ...
                "disk_cache_size": len(list(Path(self.vlm_cache.cache_dir).glob("*.pkl"))),
    ...
    def clear_all_caches(self) -> Dict[str, int]:
        ...
        total_agent_items = agent_stats["memory_cache_size"] + agent_stats["disk_cache_size"]
    ...
    def log_cache_report(self):
        ...
        logger.info(f"[CHE] LLM: {llm_stats['memory_cache_size']}mem + {llm_stats['disk_cache_size']}disk items")
        
        # Agent Cache
        agent_stats = stats["agent_cache"]
        logger.info(f"[CHE] Agent: {agent_stats['memory_cache_size']}mem + {agent_stats['disk_cache_size']}disk items")
        
        # VLM Cache
        vlm_stats = stats["vlm_cache"]
        logger.info(f"[CHE] VLM: {vlm_stats['memory_cache_size']}mem + {vlm_stats['disk_cache_size']}disk items")
    ...
    def print_cache_report(self):
        ...
        # Totals
        total_memory = (llm_stats['memory_cache_size'] + 
                       agent_stats['memory_cache_size'] + 
                       vlm_stats['memory_cache_size'])
        total_disk = (llm_stats['disk_cache_size'] + 
