...
class LLMCache:
    ...
    def _get_request_hash(
        ...
        try:
            ...
            if isinstance(messages, str):
                ...
                content = json.dumps([
                    {"type": getattr(msg, "__class__", {}).get("__name__", "unknown"), 
            ...
            return hashlib.sha256(request_string.encode()).hexdigest()[:SystemLimits.HASH_LENGTH]
            
        except Exception as e:
            logger.warning(f"Failed to hash LLM request: {e}")
            # Fallback hash
            return hashlib.sha256(str(messages).encode()).hexdigest()[:SystemLimits.HASH_LENGTH]
    ...
    def get(
        ...
        cache_key = self._get_request_hash(messages, model, temperature, max_tokens, extra_params)
    ...
    def set(
        ...
        if not self._is_cacheable(response):
            logger.debug(f"Skipping cache for non-serializable response (type: {type(response)})")
            return
        ...
        if len(cache_files) > self.max_size:
            ...
            for file_path in files_to_remove:
                try:
                    ...
                    logger.warning(f"Failed to remove agent cache file {file_path}: {e}")
