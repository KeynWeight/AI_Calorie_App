...
class NutritionService:
    ...
    def __init__(
        ...
        # API call tracking
        self.api_call_counts = {"llm_calls": 0, "mcp_calls": 0, "agent_calls": 0}
            ...
            if self.mcp_config:
                ...
                try:
                    if self.agent_api_key and self.agent_base_url:
                        ...
                        logger.info(
                            f"Created LangChain LLM for MCP agent: {self.agent_model}"
                        )
                        if self.agent_model != self.llm_model:
                            logger.info(
                                f"Using dedicated agent model: {self.agent_model} (different from main LLM: {self.llm_model})"
                            )
                except ImportError:
                    logger.warning(
                ...
                self.mcp_service = await MCPNutritionManager.get_service(
                    mcp_config=self.mcp_config, llm=agent_llm, agent_config=agent_config
                )
                ...
                if self._mcp_connected and agent_llm:
                    ...
                    logger.info(
                        f"MCP service initialized with AI agent: {agent_status['agent_available']}"
                    )
                else:
                    logger.info(
                        f"MCP service initialized: {self._mcp_connected} (no agent)"
                    )
    ...
    def apply_user_modifications(
        self, original_analysis: DishNutrition, modifications: UserModification
    ) -> DishNutrition:
        """
        ...
        try:
            ...
            logger.info(
                f"[USER MODIFICATION REQUEST] User requested changes to: '{original_analysis.dish_name}'"
            )
            logger.info(
                f"[USER MOD] Original Analysis: {original_analysis.total_calories} calories, {len(original_analysis.ingredients)} ingredients"
            )
            logger.info(
                f"[USER MOD] Modifications requested: {modifications.model_dump(exclude_none=True)}"
            )

            # Log each type of modification requested
            if modifications.dish_name:
                logger.info(
                    f"[USER MOD] Dish name change: '{original_analysis.dish_name}' -> '{modifications.dish_name}'"
                )
            if modifications.portion_size:
                logger.info(
                    f"[USER MOD] Portion size change: '{original_analysis.portion_size}' -> '{modifications.portion_size}'"
                )
            if modifications.ingredients_to_remove:
                logger.info(
                    f"[USER MOD] Remove {len(modifications.ingredients_to_remove)} ingredients: {modifications.ingredients_to_remove}"
                )
            if modifications.ingredients_to_add:
                logger.info(
                    f"[USER MOD] Add {len(modifications.ingredients_to_add)} ingredients: {modifications.ingredients_to_add}"
                )
            if modifications.ingredient_weight_changes:
                logger.info(
                    f"[USER MOD] Weight changes for {len(modifications.ingredient_weight_changes)} ingredients:"
                )
                for (
                    ingredient,
                    new_weight,
                ) in modifications.ingredient_weight_changes.items():
                    # Find original weight
                    original_weight = next(
                        (
                            ing.weight
                            for ing in original_analysis.ingredients
                            if ing.ingredient.lower() == ingredient.lower()
                        ),
                        "unknown",
                    )
                    logger.info(
                        f"[USER MOD]   {ingredient}: {original_weight}g -> {new_weight}g"
                    )
            ...
            if modifications.dish_name:
                ...
                logger.debug(
                    f"[USER MOD] Dish name: '{old_name}' -> '{modified.dish_name}'"
                )
            ...
            if modifications.portion_size:
                ...
                logger.debug(
                    f"[USER MOD] Portion size: '{old_portion}' -> '{modified.portion_size}'"
                )
            ...
            if modifications.ingredients_to_remove:
                ...
                ingredients_to_keep = []
                remove_set = {
                    remove.lower() for remove in modifications.ingredients_to_remove
                }
                ...
                logger.debug(
                    f"[USER MOD] Removed {removed_count} ingredients: {removed_ingredients}"
                )
            ...
            if modifications.ingredient_weight_changes:
                for (
                    ...
                    self._update_ingredient_weight(
                        modified.ingredients, ingredient_name, new_weight
                    )
            ...
            if modifications.ingredients_to_add:
                new_ingredients = []
                logger.info(
                    f"[USER MOD] Processing {len(modifications.ingredients_to_add)} new ingredients to add..."
                )

                for ingredient_name, weight in modifications.ingredients_to_add.items():
                    logger.info(
                        f"[LLM REQUEST] Requesting nutrition data for new ingredient: '{ingredient_name}' ({weight}g)"
                    )
                    ...
                    if ingredient:
                        ...
                        logger.info(
                            f"[LLM RESPONSE] Successfully got nutrition for '{ingredient_name}':"
                        )
                        ...
                        logger.info(
                            f"[LLM RESPONSE]   Carbs: {ingredient.carbohydrates}g"
                        )
                        logger.info(f"[LLM RESPONSE]   Fat: {ingredient.fat}g")
                        if ingredient.sodium:
                            logger.info(
                                f"[LLM RESPONSE]   Sodium: {ingredient.sodium}mg"
                            )
                    else:
                        logger.error(
                            f"[LLM ERROR] Failed to get nutrition data for ingredient: {ingredient_name}"
                        )

                modified.ingredients.extend(new_ingredients)
                logger.info(
                    f"[USER MOD] Successfully added {len(new_ingredients)} new ingredients to dish"
                )
            ...
            logger.info(
                f"[NUTRITION COMPARISON]   Dish Name: '{original_analysis.dish_name}' -> '{modified.dish_name}'"
            )
            logger.info(
                f"[NUTRITION COMPARISON]   Ingredients: {len(original_analysis.ingredients)} -> {len(modified.ingredients)} items"
            )
            logger.info(
                f"[NUTRITION COMPARISON]   Calories: {old_calories} -> {new_calories} ({calorie_change:+d})"
            )
            logger.info(
                f"[NUTRITION COMPARISON]   Protein: {old_protein:.1f}g -> {new_protein:.1f}g ({protein_change:+.1f}g)"
            )
            logger.info(
                f"[NUTRITION COMPARISON]   Carbs: {old_carbs:.1f}g -> {new_carbs:.1f}g ({carbs_change:+.1f}g)"
            )
            logger.info(
                f"[NUTRITION COMPARISON]   Fat: {old_fat:.1f}g -> {new_fat:.1f}g ({fat_change:+.1f}g)"
            )
            logger.info(
                f"[NUTRITION COMPARISON]   Sodium: {old_sodium:.1f}mg -> {new_sodium:.1f}mg ({sodium_change:+.1f}mg)"
            )
            ...
            if old_calories > 0:
                ...
                if abs(calorie_pct_change) > 5:  # Log if >5% change
                    logger.info(
                        f"[NUTRITION IMPACT] Significant calorie change: {calorie_pct_change:+.1f}%"
                    )

            logger.info(
                f"[USER MOD COMPLETE] Final dish: '{modified.dish_name}' - {new_calories} calories ({len(modified.ingredients)} ingredients)"
            )
    ...
    def _update_ingredient_weight(
        self, ingredients: List[Ingredient], ingredient_name: str, new_weight: float
    ):
        ...
        for ingredient in ingredients:
            if ingredient.ingredient.lower() == ingredient_name.lower():
                ...
                if old_weight_grams > 0:
                    ...
                    ingredient.carbohydrates = round(
                        ingredient.carbohydrates * ratio, 2
                    )
                    ...
                    logger.info(
                        f"[WEIGHT CHANGE] Modified ingredient: {ingredient_name}"
                    )
                    logger.info(
                        f"[WEIGHT CHANGE]   Weight: {old_weight_grams}g -> {new_weight_grams}g (ratio: {ratio:.2f})"
                    )
                    logger.info(
                        f"[WEIGHT CHANGE]   Calories: {old_calories} -> {ingredient.calories} ({ingredient.calories - old_calories:+d})"
                    )
                    logger.info(
                        f"[WEIGHT CHANGE]   Protein: {ingredient.protein:.1f}g, Carbs: {ingredient.carbohydrates:.1f}g, Fat: {ingredient.fat:.1f}g"
                    )
                break

    def validate_nutrition_data(
        self, nutrition_data: DishNutrition
    ) -> ValidationResult:
        """
    ...
    def _validate_critical_data(
        self, nutrition_data: DishNutrition, result: ValidationResult
    ):
    ...
    def _validate_consistency(
        ...
        if carb_diff > 10:
            result.warnings.append(
                f"Carbohydrate mismatch: {carb_diff:.1f}g difference"
            )
        if protein_diff > 10:
    ...
    def _validate_ingredients(
        ...
        for ing in nutrition_data.ingredients:
            ...
            if weight_grams <= 0:
                result.warnings.append(
                    f"Invalid weight for {ing.ingredient}: {weight_grams}g"
                )
                continue
            ...
            if weight_grams > 500:  # Over 500g for single ingredient
                ...
                weight_grams > 300 and calories_per_100g > 200
            ):  # High weight + high calorie density
                result.warnings.append(
    ...
    def _generate_health_suggestions(
        ...
        if total_calories > 800:
            result.suggestions.append(
                "High calorie content - consider smaller portions or sharing"
            )
        elif total_calories < 200:
            result.suggestions.append(
                "Low calorie content - may need additional sides for a full meal"
            )

        # Protein assessment
        if total_protein > 30:
            result.suggestions.append(
                "Excellent protein content for muscle health and satiety"
            )
        elif total_protein < 10:
            result.suggestions.append(
                "Low protein content - consider adding protein sources like meat, fish, or legumes"
            )

        # Sodium assessment
        if total_sodium > 1500:
            result.suggestions.append(
                "High sodium content - consider reducing salt or choosing low-sodium alternatives"
            )
        ...
        if total_fiber > 8:
            ...
            result.suggestions.append(
                "Low fiber content - consider adding vegetables, fruits, or whole grains"
            )

        # Ingredient variety
        if ingredient_count >= 6:
            result.suggestions.append(
                "Great ingredient variety provides diverse nutrients"
            )
        elif ingredient_count <= 2:
            result.suggestions.append(
                "Simple dish - consider adding vegetables for more nutrients"
            )
    ...
    async def match_ingredients_with_usda_efficient(
        self, ingredients: List, dish_context: str = ""
    ) -> List:
        """
        ...
        logger.info(
            f"AI Agent: Starting EFFICIENT USDA matching for {len(ingredients)} ingredients"
        )
        ...
        try:
            ...
            logger.info(
                f"[LLM CALL] Extracting keywords from {len(ingredient_names)} ingredients: {ingredient_names}"
            )
            keywords = await self._extract_keywords_from_ingredients(ingredient_names)
            logger.info(
                f"AI extracted {len(keywords)} unique keywords: {list(keywords)}"
            )
            ...
            for keyword in keywords:
                ...
                logger.info(
                    f"[MCP RESULT] Found {len(results)} results for '{keyword}'"
                )

            # Step 3: Use AI agent ONCE to match all ingredients against all results
            logger.info(
                "[AGENT CALL] Using AI agent to match all ingredients against search results..."
            )
            ...
            logger.info(
                f"[API SUMMARY] Total API calls: {total_calls} (LLM: {self.api_call_counts['llm_calls']}, MCP: {self.api_call_counts['mcp_calls']}, Agent: {self.api_call_counts['agent_calls']})"
            )
            logger.info(
                f"Efficient matching complete: {successful_matches}/{len(ingredients)} ingredients enhanced"
            )
            ...
            logger.error(
                f"Efficient matching failed: {str(e)}, using original ingredients"
            )
            return ingredients

    async def match_ingredients_with_usda_batch(
        self, ingredients: List, dish_context: str = ""
    ) -> List:
        """
        ...
        if not agent_status.get("agent_available", False):
            logger.warning(
                "AI agent not available, falling back to individual processing"
            )
            return await self.match_ingredients_with_usda_individual(
                ingredients, dish_context
            )

        logger.info(
            f"AI Agent: Starting BATCH ingredient matching for {len(ingredients)} ingredients"
        )
        logger.info(f"Dish context: {dish_context}")
        ...
        try:
            ...
            if self.mcp_service.agent:
                ...
                logger.info(
                    "Batch processing currently disabled, falling back to individual processing"
                )
                return await self.match_ingredients_with_usda_individual(
                    ingredients, dish_context
                )
    ...
    async def match_ingredients_with_usda_individual(
        self, ingredients: List, dish_context: str = ""
    ) -> List:
        """
        ...
        logger.info(
            f"AI Agent: Starting individual ingredient matching for {len(ingredients)} ingredients"
        )
        logger.info(f"Dish context: {dish_context}")
        ...
        for ingredient in ingredients:
            try:
                ...
                if self.mcp_service.agent:
                    usda_match = await self.mcp_service.agent.find_best_match(
                        ingredient_name
                    )
                else:
                    logger.info(
                        f"Using rule-based search (no agent) for: '{ingredient_name}'"
                    )
                    # Fallback to rule-based search if agent not available
                    usda_match = await self.mcp_service._find_best_ingredient_match(
                        ingredient_name
                    )
                ...
                if usda_match:
                    ...
                    logger.info(
                        f"✅ Agent found match: {usda_match.get('description', 'Unknown')} (ID: {food_id})"
                    )
                    ...
                    if usda_nutrition:
                        ...
                        calories_nutrient = usda_nutrition.get_nutrient_by_name(
                            "Energy"
                        )
                        protein_nutrient = usda_nutrition.get_nutrient_by_name(
                            "Protein"
                        )
                        fat_nutrient = usda_nutrition.get_nutrient_by_name(
                            "Total lipid (fat)"
                        )
                        carbs_nutrient = usda_nutrition.get_nutrient_by_name(
                        ...
                        if protein_nutrient:
                            enhanced_ingredient.protein = round(
                                protein_nutrient.amount, 2
                            )
                        if fat_nutrient:
                            enhanced_ingredient.fat = round(fat_nutrient.amount, 2)
                        if carbs_nutrient:
                            enhanced_ingredient.carbohydrates = round(
                                carbs_nutrient.amount, 2
                            )
                        ...
                        logger.info(
                            f"    AI Agent result: {usda_match.get('description', '')[:60]}..."
                        )
                        logger.info(
                            f"    USDA ID: {food_id}, Total nutrients: {len(usda_nutrition.nutrients)}"
                        )
                        logger.info(
                            f"    Calories: {ingredient.calories} -> USDA: {enhanced_ingredient.calories}"
                        )
                        if hasattr(usda_match, "agent_reasoning"):
                            logger.info(
                                f"    Agent reasoning: {usda_match.get('agent_reasoning', '')[:100]}..."
                            )
                        logger.info(
                            f"    Complete nutrition data with {len(usda_nutrition.nutrients)} nutrients via MCP tools"
                        )
                    else:
                        logger.warning(
                            f"AI Agent found match but failed to get nutrition data for {ingredient_name}"
                        )
                        enhanced_ingredients.append(ingredient)
                else:
                    logger.warning(
                        f"AI Agent could not find USDA match for: {ingredient_name}"
                    )
                    enhanced_ingredients.append(ingredient)

            except Exception as e:
                logger.error(
                    f"Error matching ingredient '{ingredient.ingredient}': {str(e)}"
                )
                enhanced_ingredients.append(ingredient)  # Keep original on error

        logger.info(
            f"Individual matching complete: {successful_matches}/{len(ingredients)} ingredients enhanced with USDA data"
        )

        if self.mcp_service.agent and hasattr(self.mcp_service.agent, "api_calls_made"):
            logger.info(
                f"Agent API calls this session: {self.mcp_service.agent.api_calls_made}"
            )
            logger.info(
                f"Cached mappings: {len(self.mcp_service.agent.successful_mappings) if self.mcp_service.agent.successful_mappings else 0}"
            )

        return enhanced_ingredients

    def _parse_batch_agent_result(
        self, batch_result: str, ingredient_names: List[str]
    ) -> Dict:
    ...
    async def _process_usda_match(self, ingredient, usda_match: Dict) -> Optional:
        ...
        try:
            ...
            if usda_nutrition:
                ...
                logger.warning(
                    f"AI Agent found match but failed to get nutrition data for {ingredient.ingredient}"
                )
                return None

        except Exception as e:
            logger.error(
                f"Error processing USDA match for {ingredient.ingredient}: {str(e)}"
            )
            return None

    async def _extract_keywords_from_ingredients(
...
Format: KEYWORDS: keyword1, keyword2, keyword3, ..."""
            ...
            logger.debug(
                f"[LLM CALL] Extracting keywords from {len(ingredient_names)} ingredients"
            )
            ...
            if response and "KEYWORDS:" in response:
                ...
                logger.info(
                    f"AI extracted {len(keywords)} keywords from {len(ingredient_names)} ingredients"
                )
    ...
    async def _match_ingredients_with_agent(
        self, ingredients: List, keyword_results: dict, dish_context: str
    ) -> List:
        """
...
"""
        ...
        try:
            ...
            logger.info(
                f"[AGENT] Using batch agent to match {len(ingredients)} ingredients"
            )
            agent_result = await self.mcp_service.agent.find_batch_matches(
                matching_prompt
            )
    ...
    async def _parse_agent_matching_result(
        self, agent_result: str, ingredients: List, usda_id_lookup: dict
    ) -> List:
        ...
        try:
            logger.info(
                f"[AGENT PARSE] Parsing agent JSON result (length: {len(agent_result)})"
            )
            ...
            try:
                ...
                if json_start >= 0 and json_end > json_start:
                    ...
                    logger.info(
                        f"[AGENT PARSE] Successfully parsed JSON with {len(matches)} matches"
                    )
                else:
            ...
            for match_data in matches:
                try:
                    ...
                    if not ingredient:
                        logger.warning(
                            f"[AGENT PARSE] No ingredient found for match: {ingredient_name}"
                        )
                        continue
                    ...
                    if matched:
                        ...
                        logger.info(
                            f"[AGENT PARSE] Processing match: {ingredient_name} -> ID:{fdc_id} ({confidence})"
                        )
                        ...
                        if fdc_id and fdc_id in usda_id_lookup:
                            ...
                            logger.info(
                                f"[AGENT PARSE] Attempting to enhance {ingredient_name} with USDA data"
                            )
                            ...
                            if enhanced:
                                ...
                                logger.info(
                                    f"[AGENT PARSE] SUCCESS: {ingredient_name} -> {description} (ID: {fdc_id})"
                                )
                            else:
                                logger.warning(
                                    "[AGENT PARSE] Failed to enhance ingredient with USDA data"
                                )
                        else:
                            logger.warning(
                                f"[AGENT PARSE] USDA ID {fdc_id} not found in lookup table"
                            )
                            logger.debug(
                                f"[AGENT PARSE] Available IDs: {list(usda_id_lookup.keys())[:5]}..."
                            )
                    else:
                        reason = match_data.get("reason", "No match found")
                        logger.info(
                            f"[AGENT PARSE] Agent: NO_MATCH for {ingredient_name} - {reason}"
                        )
                    ...
                    logger.error(
                        f"[AGENT PARSE] Error processing individual match: {e}"
                    )
            ...
            for ingredient in ingredients:
                if ingredient.ingredient.lower() not in processed_ingredients:
                    logger.warning(
                        f"[AGENT PARSE] Ingredient not found in agent matches: {ingredient.ingredient}"
                    )
                    enhanced_ingredients.append(ingredient.model_copy(deep=True))
            ...
            logger.info(
                f"[AGENT PARSE] JSON parsing complete: {successful_enhancements}/{len(ingredients)} ingredients enhanced"
            )

        except Exception as e:
            logger.error(
                f"[AGENT PARSE] Error parsing agent JSON matching result: {str(e)}"
            )
            return ingredients
    ...
    async def _enhance_ingredient_with_usda_data(self, ingredient, usda_result: dict):
        ...
        try:
            ...
            logger.info(
                f"[ENHANCE] Getting complete nutrition data for {ingredient.ingredient} (ID:{food_id})"
            )
            ...
            if usda_nutrition:
                ...
                logger.info(
                    f"[ENHANCE] Successfully enhanced {ingredient.ingredient} with USDA nutrition data"
                )
                logger.info(
                    f"[ENHANCE] USDA data applied - Calories: {enhanced_ingredient.calories}, Protein: {enhanced_ingredient.protein}g, Fat: {enhanced_ingredient.fat}g, Carbs: {enhanced_ingredient.carbohydrates}g"
                )

                # Log available micronutrients
                if enhanced_ingredient.complete_nutrition:
                    nutrient_count = len(
                        enhanced_ingredient.complete_nutrition.nutrients
                    )
                    logger.info(
                        f"[ENHANCE] Complete nutrition data includes {nutrient_count} nutrients:"
                    )
                    for (
                        nutrient_id,
                        nutrient_info,
                    ) in enhanced_ingredient.complete_nutrition.nutrients.items():
                        logger.debug(
                            f"  - {nutrient_info.name}: {nutrient_info.amount} {nutrient_info.unit}"
                        )

                return enhanced_ingredient
            else:
                logger.warning(
                    f"[ENHANCE] No nutrition data available for {ingredient.ingredient}"
                )

        except Exception as e:
            logger.error(
                f"[ENHANCE] Error enhancing ingredient with USDA data: {str(e)}"
            )
